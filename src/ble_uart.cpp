...
// This will help with issues where data is overwritten while its being
// processed Cost:
...
static void Error_Handler() { asm("bkpt 255"); }
...
void UartHandler::Init()
{
	...
	uhandle.huart1.Init.BaudRate = 9600;
	...
	uhandle.huart1.Init.HwFlowCtl = UART_HWCONTROL_RTS_CTS;
	...
	if (HAL_UARTEx_SetTxFifoThreshold(&uhandle.huart1,
	                                  UART_TXFIFO_THRESHOLD_1_8) != HAL_OK)
	{
		Error_Handler();
	}
	if (HAL_UARTEx_SetRxFifoThreshold(&uhandle.huart1,
...
int UartHandler::StartRx()
{
	...
	// Now start Rx
	status =
	    HAL_UART_Receive_DMA(&uhandle.huart1,
...
bool UartHandler::RxActive() { return uhandle.rx_active; }
...
{
	return HAL_UART_Transmit(&uhandle.huart1, (uint8_t*)buff, size, 100);
}

int UartHandler::CheckError() { return HAL_UART_GetError(&uhandle.huart1); }
